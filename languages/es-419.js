{
	"Edit": "Editar",
	"Server List": "Lista de Servidores",
	"Settings": "Opciones",
	"Update": "Actualizar",
	"Reload": "Recargar",
	"Add New Server": "Añadir Nuevo Servidor",
	"Connect to Server": "Conectarse al Servidor",
	"Disconnect from Server": "Desconectarse del Servidor",
	"Remove": "Eliminar",
	"Online": "En Linea",
	"Playing": "Jugadores",
	"This will remove the server": "Esto eliminara el servidor",
	"from the list permanently": "de la lista permanentemente",
	"Delete":"Eliminar",
	"WARNING!!!":"¡¡¡ADVERTENCIA!!!",
	"Version": "Version",
	"Server Name": "Nombre del Servidor",
	"Server URL": "URL del Servidor",
	"Country": "Nacionalidad",
	"New Server Information":"Datos Del Nuevo Servidor",
	"No Country": "Sin Nacionalidad",
	"Close": "Cerrar",
	"Add Server": "Añadir Servidor",
	"Edit Server": "Editar Servidor",
	"Save": "Guardar",
	"Main Configuration": "Configuracion Principal",
	"Network Adapter": "Adaptador de Red",
	"Main Configuration Text": "Esta es la configuracion general, asegurate de seleccionar donde esta lan play antes que nada.",
	"Lan Play Location": "Localizacion de Lan Play: ",
	"Browse": "Buscar",
	"Fake Internet": "Internet Falso: ",
	"Broadcast": "Broadcast: ",
	"PMTU": "PMTU",
	"Ok": "Aceptar",
	"Apply": "Aplicar",
	"Cancel": "Cancelar",
	"Network Adapter Text": "Aqui seleccionar tu interfaz de red para que cuando te conectes a un servidor no tengas que seleccionar la interfaz manualmente. Primero Clicka en el boton de Cargar Interfaces y selecciona la que corresponda a tu IP Local",
	"Load Interfaces": "Cargar Interfaces",
	"Load Interfaces First": "Carga las Interfaces Primero",
	"No Network Interface": "Sin Interfaz de Red Seleccionada",
	"Server List Text": "Aqui puedes exportar e importar tu lista de servidores de/a un archivo local o descargar e importar la lista oficial desde github.",
	"Export Server List": "Exportar Lista de Servidores",
	"Import Server List": "Importar Lista de Servidores",
	"Download Offical Server List": "Descargar Lista Oficial de Servidores",
	"Are you sure?": "¿Estas Seguro?",
	"Are you sure? Text": "Esto borrara tu lista de servidores actual y la remplazara con la oficial ¿estas seguro de querer eso?",
	"Are you sure?2 Text": "Esto borrara tu lista de servidores actual y la remplazara con la de lan-play.com ¿estas seguro de querer eso?",
	"Download lan-play.com's Server List": "Descargar La lista de Servidores de lan-play.com",
	"No! Let me go back!": "¡No! ¡Atras!",
	"I am okay whit that, replace it!": "Estoy conforme con ello¡Remplazala!",
	"Enable Proxy:": "Activar Proxy: ",
	"Success!!": "¡Todo Correcto!",
	"Success!! Text" : "¡La operacion fue todo un exito!",
	"Offline" : "Desconectado",
	"Not Online" : "No Online",
	"Error 404: Lan Play not found!" : "Error 404: Lan Play no found!",
	"Error 403: Update Lan Play to the lastest version" : "Error 403: Actualiza Lan Play a la ultima version",
	"ERROR!":"¡ERROR!",
	"Error 1 text":"La version de Lan Play esta desactualizada, porfavor actualize a la version mas reciente en este lugar:",
	"Error 2 text":"Necesistas descargar Lan Play y configurar en Opciones donde lo tienes almacenado, puedes descargar Lan Play aqui:",
	"NEW UPDATE AVALIABLE" : "NUEVA ACTUALIZACION DISPONIBLE",
	"NEW UPDATE AVALIABLE text" : "Tu tienes la version $VERSION_NUMBER$ de la GUI y la nueva version %VERSION_NUMBER% de la GUI ha sido publicada, porfavor actualize.",
	"Download Update" : "Descargar Actualizacion"
}